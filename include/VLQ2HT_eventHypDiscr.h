    ...
    virtual bool process(Event & event) override {
        ...
        for(auto & h_hyp: h_hyps) {
            for(auto & t_hyp: t_hyps) {
                ...
                // "remove" hypotheses where ak4 jet overlapps with higgs ak8
                for (const auto & t_hyp_jet: t_hyp.toplep_jets) {
                    if (deltaR(t_hyp_jet.v4(), h_hyp.higgs_v4) < 0.9) {
                        chi2 = 99999.;
                        break;
                    }
                }

                // calc chi2, if no overlapping jets are found
                if (chi2 < 0) {
        ...
        float chi2 = 99999.;
        const auto & best_h_hyp = get_best_hypothesis(h_hyps, "Chi2_event", chi2);
        const auto & best_t_hyp = get_best_hypothesis(t_hyps, "Chi2_event", chi2);
        if (chi2 < 99998.) {
            if (best_h_hyp) {
                event.set(h_higgs, best_h_hyp->higgs_v4);
                if (best_h_hyp->higgs_jets.size()) {
                    auto j = best_h_hyp->higgs_jets[0];
                    event.set(h_jet, best_h_hyp->higgs_jets);
